...
def training(model:nn.Module, optimizer:nn.Optimizer, train_loader:Dataset, now_epoch:int, num_epochs:int):
    ...
    for data in pbar:
        ...
        all_losses = compute_total_loss(model,scores, score_mat, label,
                                  rich_feat, fused_feat,
                                  domain_logits=None, domain_labels=None,
                                  mask=mask, gcn_feat=model._d_feats["d4"])
...
def run(model:nn.Module, optimizer:nn.Optimizer, train_loader:Dataset, val_loader:Dataset, num_epochs:int, modelroot:str, savename:str):
    best_acc = 0
    for epoch in range(num_epochs):
        ...
        if acc > best_acc:
            best_acc = acc
            model.save(os.path.join(modelroot, savename))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckpt", type=str, default="pertrain_checkpoint1.pkl",help="save checkpoint path")
    ...
    if not args.testonly:
        ...
        test_set =BreastCSVSet(args.csv_path, mode='val', transform=transform_val, augment=False,
    ...
    model = Res101_Mamba_ITE_UNet_GraphPlus(num_classes=num_classes,in_channels=4,freeze_seg=True, seg_ckpt=args.seg_ckpt)
