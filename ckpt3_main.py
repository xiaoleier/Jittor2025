...
class Net2(nn.Module):
    def __init__(self, num_classes=6, pretrain=True, loadfrom=None):
        super().__init__()
        self.base_net = Res101_Mamba_ITE_UNet_GraphPlus(num_classes=num_classes,in_channels=3,freeze_seg=True, seg_ckpt="ckpts/emcadnet_best_modelv4.pkl")  # 先随便 1000
        if loadfrom:
            load_weights_flexible(self.base_net, loadfrom)
    ...
    def extract_feat(self, x):
        ...
        outs = self.base_net(x)


        # x = self.neck(outs['fused_feat'])
        return outs
...
def training(model:nn.Module, optimizer:nn.Optimizer, train_loader:Dataset, now_epoch:int, num_epochs:int):
    ...
    for data in pbar:
        ...
        loss, loss_cls, loss_score, loss_struct_align, loss_struct_center, loss_uncertainty = compute_total_loss(
                model.base_net, outs['scores'], outs['score_mat'], label,
        ...
        #     model.base_net, outs[1], outs[2], label,
        #     outs[3], outs[0], gcn_feat=outs[4],
        #     num_classes=6)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loadpertain', type=str, default='./model_save/pertrain_checkpoint3.pkl')
    parser.add_argument("--ckpt", type=str, default="checkpoint3.pkl", help="save checkpoint path")
    parser.add_argument('--loadfrom', type=str, default='./model_save/checkpoint3.pkl')
    ...
    # model = Net2(num_classes=6, pretrain=True)
