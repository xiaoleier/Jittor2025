...
class ScoreHead(nn.Module):
    ...
    def __init__(self, in_feat=2048, num_classes=6, p=0.5):
        super().__init__()
        self.cls    = nn.Linear(256, num_classes*5)
        self.table = jt.arange(1, 5 + 1).float32()

    def score_softmax(self, logits):
        B = logits.shape[0]
        return nn.softmax(logits.reshape(B,6, 5), dim=2)
...
class Net2(nn.Module):
    def __init__(self, num_classes=6, pretrain=True, loadfrom=None):
        super().__init__()
        self.base_net = Res101_Mamba_ITE_UNet_GraphPlus(num_classes=num_classes,in_channels=4,freeze_seg=True, seg_ckpt="ckpts/best_model.pkl")  # 先随便 1000
        if loadfrom:
            load_weights_flexible(self.base_net, loadfrom)
        ...
        self.head = ScoreHead(256, num_classes)
...
def training(model:nn.Module, optimizer:nn.Optimizer, train_loader:Dataset, now_epoch:int, num_epochs:int):
    ...
    for data in pbar:
        ...
        pbar.set_description(f'Epoch {now_epoch+1} [TRAIN] loss = {losses[-1]:.2f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loadpertain', type=str, default='./model_save/pertrain_checkpoint2.pkl')
    parser.add_argument("--ckpt", type=str, default="checkpoint2.pkl", help="save checkpoint path")

    parser.add_argument('--loadfrom', type=str, default='./model_save/checkpoint2.pkl')
    parser.add_argument('--result_path', type=str, default='./result.txt')
    args = parser.parse_args()

    # model = ConvNeXtTinyScoreNet(num_classes=6)
    # model = ConvNeXtTinyACmixScoreNet(num_classes=6)
    # model = ConvNeXtTinyITE_MambaScoreNet(num_classes=6)
    # model = ResNeXt50MultiScaleCBAM(num_classes=6)
    model = Net2(num_classes=6, pretrain=True,loadfrom=args.loadpertain)
    # model = ResNet101_ITE_Mamba_Score(num_classes=6)
    # model = UNetScoreSoftmax(num_classes=6)
    # model = ResNet101_Fusion_UNet_Score(num_classes=6)
    # model = Res101_Mamba_ITE_UNet_GraphPlus(num_classes=6)
    # model = ResNet101(num_classes=6, pretrain=True,in_channels=4)
    ...
    #     transform.Resize((512, 512)),  # ① 先缩放到统一分辨率
    #     transform.RandomCrop(448),  # ② 随机裁剪局部
    #     transform.RandomHorizontalFlip(p=0.5),  # ③ 水平翻转
    #     transform.RandomRotation(15),  # ④ ±15° 小角旋
    #     transform.ColorJitter(brightness=0.1,  # ⑤ 亮度 & 对比度扰动
    #                           contrast=0.2),
    #     transform.ToTensor(),  # ⑥ HWC→CHW & /255
    #     transform.ImageNormalize(mean=[0.5] * 3,  # ⑦ 归一化到 [-1,1]
    #                              std=[0.5] * 3),
    # ])
    #
    # # 验证 / 测试集
    ...
    #     transform.ToTensor(),
    #     transform.ImageNormalize(mean=[0.5] * 3, std=[0.5] * 3),
    # ])
