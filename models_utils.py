...
class MskCrossAttentionFusion(nn.Module):
    ...
    def __init__(self, embed_dim=2048, dropout=0.1):
        ...
        self.k_proj = nn.Linear(256 + 512 + 1024+128, embed_dim)
        self.v_proj = nn.Linear(256 + 512 + 1024+128, embed_dim)
...
class Res101_Mamba_ITE_UNet_GraphPlus(nn.Module):
    def __init__(self, num_classes=Config.num_classes,in_channels=3,freeze_seg=True, seg_ckpt="busi_seg_ckpt.pkl"):
        ...
        self.dir_attn_c4 = DirectionalAttention(in_channels=256, height=14, width=14)
    ...
    def execute(self, x, return_domain=False):
        ...
        # d4 = nn.AdaptiveAvgPool2d((1, 1))(self._d_feats["d4"]).squeeze(-1).squeeze(-1)
        drop_feat = jt.concat([d1, d2, d3], dim=1)
        # drop_feat = self.proj_droped(drop_feat)
        ...
        c4 = self.dir_attn_c4(c4)  # 方向注意力

        u = self._d_feats["d4"] # [B,128,H_unet,W_unet]
        u=self.proj_unet(u)
        h, w = c2.shape[2:]
...
class ContourTokenCrossAttention(nn.Module):
    def __init__(self, dim=512, num_heads=4):
        super().__init__()
        self.contour_token = nn.Parameter(jt.randn(1, 1, dim))
