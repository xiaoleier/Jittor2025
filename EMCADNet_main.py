...
def deep_supervision_loss(preds: Tuple[jt.Var, ...], target: jt.Var, weights=(1.0,0.75, 0.5, 0.25)) -> jt.Var:
    loss_fn = DiceCELoss()
...
def train(model: nn.Module, loader: jt.dataset.Dataset, val_loader: jt.dataset.Dataset, ckpt: str, epochs: int = 150, lr: float = 1e-3, out_dir: str = "ckpts"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader:
            ...
            if val_loss < best_val:
                ...
                # jt.save(model.state_dict(), Path(out_dir) / "best_model.pkl")
                jt.save(model.state_dict(), str(Path(out_dir) / ckpt))
...
def segment_image(
    ...
    """
    # 1) 建网络并加载权重
    net = EMCADNet(encoder='resnet101')
...
#     # parser.add_argument("--infer_mask", type=str, default="TrainSet/mask_malign", help="Path to save predicted masks")
#     # parser.add_argument("--infer_overlay", type=str, default="TrainSet/overlay_malign", help="Path to save overlay images")
#
...
#     #                     help="Path to folder of inference images (e.g., TrainSet/images/train)")
#     # parser.add_argument("--infer_mask", type=str, default="./BUSUCLM/mask", help="Path to save predicted masks")
#     # parser.add_argument("--infer_overlay", type=str, default="./BUSUCLM/overlay", help="Path to save overlay images")
#
#     parser.add_argument("--infer_images", type=str,
#                         default="./BUSUCLM/bus_uclm_separated/malign",
#                         help="Path to folder of inference images (e.g., TrainSet/images/train)")
#     parser.add_argument("--infer_mask", type=str, default="./BUSUCLM/mask_malign", help="Path to save predicted masks")
#     parser.add_argument("--infer_overlay", type=str, default="./BUSUCLM/overlay_malign", help="Path to save overlay images")
#
#     # parser.add_argument("--infer_images", type=str,
#     #                     default="../Dataset_BUSI_with_GT/malignant",
#     #                     help="Path to folder of inference images (e.g., TrainSet/images/train)")
#     # parser.add_argument("--infer_mask", type=str, default="../Dataset_BUSI_with_GT/malignant_mask", help="Path to save predicted masks")
#     # parser.add_argument("--infer_overlay", type=str, default="../Dataset_BUSI_with_GT/malignant_overlay", help="Path to save overlay images")
#
#     parser.add_argument("--ckpt", type=str, default="ckpts/emcadnet_best_modelv3_malignant.pkl", help="Path to model checkpoint")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="EMACDNet (Jittor) on BUS‑BRA")
    parser.add_argument("--csv_path", type=str, default="./datasets/BUSBRA/bus_data.csv", help="Path to BUS‑BRA root containing train/val/test folders")
    parser.add_argument("--busi_root", type=str, default="./datasets/Dataset_BUSI_with_GT", help="Path to Dataset_BUSI_with_GT")
    ...
    parser.add_argument("--ckpt", type=str, default="emcadnet_best_modelv4.pkl",
    ...
    # model
    net = EMCADNet(encoder='resnet101')
    # load_weights_flexible(net, args.ckpt)

    # training
    train(net, train_loader, val_loader, epochs=args.epochs, lr=args.lr, ckpt=args.ckpt)
